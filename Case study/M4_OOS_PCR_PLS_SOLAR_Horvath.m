%% ============================================================
%  Real-data OOS validation: Horvath residual acceleration
...
%  Outputs:
%      Output_OOS/
%        Output_from_M4_Horvath_OOS_PCR.csv
%        Output_from_M4_Horvath_OOS_PLS.csv
%        Output_from_M4_Horvath_OOS_SOLAR.csv
%        Output_from_M4_Horvath_OOS_combined_metrics.csv
%
%  Metrics saved:
%      RMSE, correlation, R2
% ============================================================
...
fprintf('\n============================================================\n');
fprintf('Real-data OOS validation: Horvath residual acceleration\n');
...
outcome_name = 'AgeAccelResidual_Horvath';
...
fprintf('\n[Step 3] Filtering missing Horvath residual outcomes...\n');

valid_idx = ~isnan(w_raw);

if any(~valid_idx)
    fprintf('Removing %d samples with missing Horvath residual acceleration.\n', sum(~valid_idx));
    X = X(valid_idx, :);
    w_raw = w_raw(valid_idx);
    pheno = pheno(valid_idx, :);
else
    fprintf('No missing Horvath residual outcomes detected.\n');
end
...
writetable( ...
    split_tbl, ...
    fullfile(out_dir, 'Output_from_M4_Horvath_OOS_train_test_split.csv') ...
);
...
writetable( ...
    result_pcr, ...
    fullfile(out_dir, 'Output_from_M4_Horvath_OOS_PCR.csv') ...
);
...
writetable( ...
    result_pls, ...
    fullfile(out_dir, 'Output_from_M4_Horvath_OOS_PLS.csv') ...
);
...
writetable( ...
    result_solar, ...
    fullfile(out_dir, 'Output_from_M4_Horvath_OOS_SOLAR.csv') ...
);
...
writetable( ...
    all_results, ...
    fullfile(out_dir, 'Output_from_M4_Horvath_OOS_combined_metrics.csv') ...
);

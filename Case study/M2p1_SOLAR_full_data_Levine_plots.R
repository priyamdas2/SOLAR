# ============================================================
# Figure 2: Main SOLAR result for Levine residual acceleration
#
# Expected folder structure:
#
# Current working directory/
# ├── Output/
# │   ├── Output_from_M2_summary.csv
# │   ├── Output_from_M2_factor_strength.csv
# │   ├── Output_from_M2_latent_scores_and_fitted.csv
# │   ├── Output_from_M2_optimization_trace.csv
# │   └── Output_from_M2_top_CpG_importance.csv
...
pkg_needed <- c(
  ...
  "stringr", "scales", "forcats", "ggrepel",
  "patchwork"
)
...
summary_tbl <- read_csv(file.path(out_dir, "Output_from_M2_summary.csv"), show_col_types = FALSE)
factor_tbl  <- read_csv(file.path(out_dir, "Output_from_M2_factor_strength.csv"), show_col_types = FALSE)
score_tbl   <- read_csv(file.path(out_dir, "Output_from_M2_latent_scores_and_fitted.csv"), show_col_types = FALSE)
trace_tbl   <- read_csv(file.path(out_dir, "Output_from_M2_optimization_trace.csv"), show_col_types = FALSE)
top_cpg_tbl <- read_csv(file.path(out_dir, "Output_from_M2_top_CpG_importance.csv"), show_col_types = FALSE)
...
p_factor_strength <- ggplot(
  factor_long,
  aes(x = factor_label, y = d_squared_abs_beta_prop)
) +
  geom_col(width = 0.68, fill = col_main, alpha = 0.85) +
  geom_line(aes(y = d_squared_prop, group = 1), linewidth = 0.75, color = "black") +
  geom_point(aes(y = d_squared_prop), size = 3.1, color = "black") +
  annotate(
    "label",
    x = 2.8,
    y = 0.86,
...
ggsave(
  filename = file.path(out_dir, "Plot_M2_factor_strength.png"),
...
# Colored by Levine residual acceleration
# ============================================================

p_score_continuous <- ggplot(
  score_tbl,
  aes(x = H_factor_01, y = H_factor_02, color = y_observed)
) +
  ...
  geom_hline(yintercept = 0, linewidth = 0.35, linetype = "dashed", color = "gray65") +
  geom_vline(xintercept = 0, linewidth = 0.35, linetype = "dashed", color = "gray65") +
  labs(
    title = "SOLAR latent score map (Levine)",
...
ggsave(
  filename = file.path(out_dir, "Plot_M2_latent_score_map_colored_by_residual.png"),
...
p_score_age <- ggplot(
  score_tbl,
  aes(x = H_factor_01, y = H_factor_02, color = Age_group)
) +
...
ggsave(
  filename = file.path(out_dir, "Plot_M2_latent_score_map_colored_by_age.png"),
  plot = p_score_age,
  width = 7.2,
  height = 6.0,
  dpi = 450,
...
p_traj <- ggplot(
  traj_tbl,
  aes(x = Age_months, y = mean_score, color = factor_label, group = factor_label)
) +
  geom_hline(yintercept = 0, linewidth = 0.35, linetype = "dashed", color = "gray70") +
  geom_errorbar(
  ...
  theme(
    ...
    legend.background = element_rect(color = "gray75", fill = "white", linewidth = 0.30),
...
ggsave(
  filename = file.path(out_dir, "Plot_M2_latent_factor_developmental_profiles.png"),
  plot = p_traj,
  width = 5.1,
  height = 4.2,
  dpi = 450,
...
p_factor_box <- ggplot(
  ...
  geom_hline(yintercept = 0, linetype = "dashed", linewidth = 0.45, color = "gray55") +
  geom_violin(width = 0.85, alpha = 0.45, color = NA, trim = FALSE) +
  geom_boxplot(width = 0.16, outlier.shape = NA, fill = "white", color = "black", linewidth = 0.40) +
  facet_wrap(~ factor_label, nrow = 1, scales = "free_y") +
  labs(
    title = "Latent-factor distributions across age groups (Levine)",
...
ggsave(
  filename = file.path(out_dir, "Plot_M2_latent_factor_distributions_by_age.png"),
  plot = p_factor_box,
  width = 15.4,
  height = 4.5,
  dpi = 450,
...
# ============================================================
# Plot 6: Observed vs fitted Levine residual acceleration
# ============================================================
...
p_obs_fit <- ggplot(
  score_tbl,
  aes(x = y_fitted, y = y_observed)
) +
  geom_point(size = 1.9, alpha = 0.45, color = col_dark) +
  geom_smooth(method = "lm", formula = y ~ x, se = TRUE, linewidth = 0.85, color = "black", fill = "gray80") +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", linewidth = 0.60, color = "gray45") +
  annotate(
    "label",
    x = Inf,
    y = -Inf,
  ...
  labs(
    title = "Observed versus fitted\nresidualized Levine/DNAm PhenoAge",
...
ggsave(
  filename = file.path(out_dir, "Plot_M2_observed_vs_fitted.png"),
  plot = p_obs_fit,
  width = 5.1,
  height = 4.2,
  dpi = 450,
...
p_resid_fit <- ggplot(
  score_tbl,
  aes(x = y_fitted, y = residual)
) +
  geom_hline(yintercept = 0, linetype = "dashed", linewidth = 0.55, color = "gray50") +
  geom_point(size = 1.8, alpha = 0.42, color = col_dark) +
  geom_smooth(method = "loess", formula = y ~ x, se = TRUE, linewidth = 0.80, color = "black", fill = "gray82") +
  labs(
    title = "Residuals versus fitted values (Levine)",
    x = "Fitted residualized DNAm age (years)",
...
ggsave(
  filename = file.path(out_dir, "Plot_M2_residuals_vs_fitted.png"),
  plot = p_resid_fit,
  width = 6.4,
  height = 5.4,
  dpi = 450,
...
p_top_cpg <- ggplot(
  top_plot_tbl,
  aes(x = CpG_label, y = relative_importance)
) +
  geom_col(width = 0.72, fill = col_main, alpha = 0.88) +
  coord_flip() +
  labs(
    title = "Top CpGs by importance score (Levine)",
    x = NULL,
...
ggsave(
  filename = file.path(out_dir, "Plot_M2_top_CpG_importance_preview.png"),
  plot = p_top_cpg,
  width = 7.0,
  height = 4.8,
  dpi = 450,
...
p_loading_heat <- ggplot(
  top_heat_tbl,
  aes(x = factor_label, y = CpG_label, fill = loading)
) +
  geom_tile(color = "gray92", linewidth = 0.15) +
  ...
  theme(
    ...
    legend.background = element_rect(color = "gray70", fill = "white", linewidth = 0.45),
    legend.box.background = element_rect(color = "gray70", fill = "white", linewidth = 0.45),
...
ggsave(
  filename = file.path(out_dir, "Plot_M2_top_CpG_loading_heatmap_preview.png"),
  plot = p_loading_heat,
  width = 7.0,
  height = 4.8,
  dpi = 450,
...
img_factor <- image_read(file.path(out_dir, "Plot_M2_factor_strength.png"))
img_traj   <- image_read(file.path(out_dir, "Plot_M2_latent_factor_developmental_profiles.png"))
img_obsfit <- image_read(file.path(out_dir, "Plot_M2_observed_vs_fitted.png"))
img_dist   <- image_read(file.path(out_dir, "Plot_M2_latent_factor_distributions_by_age.png"))
...
image_write(
  fig2_combined,
  path = file.path(out_dir, "Plot_M2_combined_Figure2_panel.png"),
  format = "png",
  density = "450x450"
)

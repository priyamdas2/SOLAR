%% ============================================================
%  SOLAR real-data analysis: M1 Horvath residual acceleration
%
...
if exist('pheno_FULL', 'var') && exist('y_raw_FULL', 'var')
    ...
    if ~ismember( ...
        ...
        error(['AgeAccelResidual_Horvath column ', ...
...
if length(w_raw) ~= n
    error(['Length of Horvath residual outcome ', ...
...
method_name = 'SOLAR_Horvath_full';
...
summary_tbl.outcome = string('AgeAccelResidual_Horvath');
...
writetable( ...
    summary_tbl, ...
    fullfile(out_dir, 'Output_from_M1_summary.csv') ...
);

writetable( ...
    factor_tbl, ...
    fullfile(out_dir, 'Output_from_M1_factor_strength.csv') ...
);

writetable( ...
    score_tbl, ...
    fullfile(out_dir, 'Output_from_M1_latent_scores_and_fitted.csv') ...
);

writetable( ...
    top_tbl, ...
    fullfile(out_dir, 'Output_from_M1_top_CpG_importance.csv') ...
);

writetable( ...
    top_tbl_500, ...
    fullfile(out_dir, ...
    'Output_from_M1_top_CpG_importance_for_enrichment_500.csv') ...
);
...
writetable( ...
    top_tbl_5000, ...
    fullfile(out_dir, ...
    'Output_from_M1_top_CpG_importance_for_enrichment_5000.csv') ...
);

writetable( ...
    trace_tbl, ...
    fullfile(out_dir, 'Output_from_M1_optimization_trace.csv') ...
);
